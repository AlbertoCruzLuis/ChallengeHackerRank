...
//Date: 30/12/2019
//Problema de HackerRank - Abstract Classes - Polymorphism
...
class LRUCache : public Cache
{
    ...
    public:
        ...
        {
            ++Valor_uso;
            dat.insert(make_pair(Valor_uso,key));
            Node n(Valor_uso,value);
            head = new Node(NULL,n.prev,Valor_uso,value);
            tail = new Node(n.next,NULL,Valor_uso,value);

            if(mp.find(key) != mp.end())
            {
                mp[key]->value = value;
                mp[key]->key = Valor_uso;
            }else if(mp.size() < cp)
            {
                mp.insert(make_pair(key, head));
            }else
            {
                mp.erase(dat.begin()->second);
                mp.insert(make_pair(key, head));
            }
        }
